#Fine tuning do 2019_cat with 2019_fr
...
def main():
    ...
    with torch.no_grad():
        ...
        if f1_macro > best_f1:
            best_f1 = f1_macro
            #Salvando o modelo
            torch.save(model.state_dict(), 'model_ft_2019_cat_with_2019_fr.pt')

    #Salvar resultados para gráficos e tabelas
    np.save('ytrue_2019_cat_with_2019_fr.npy', np.array(all_true_total))
    np.save('ypred_2019_cat_with_2019_fr.npy', np.array(all_preds_total))
    np.save('epoch_loss_2019_cat_with_2019_fr.npy',np.array(epoch_loss))
    #Gerar .txt com resultados
    with open('resultado_2019_cat_with_2019_fr.txt', 'w') as f:
