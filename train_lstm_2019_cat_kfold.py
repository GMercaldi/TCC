...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        if acc > best_acc:
            best_acc = acc
            # Salvando o modelo
            torch.save(model.state_dict(), "lstm_kfold_best_2019_cat.pt")
            # Exportando o scaler
            joblib.dump(scaler, 'scaler_lstm_kfold_2019_cat.pkl')
            # Salvar encoder
            joblib.dump(linear_encoder, 'linear_encoder_2019_cat.pkl')
    ...
    np.save('ytrue_2019_cat.npy', np.array(all_true_total))
    np.save('ypred_2019_cat.npy', np.array(all_preds_total))

    # Criar .txt com resultados
    with open("resultado_2019_cat_kfold.txt", "w") as f:
