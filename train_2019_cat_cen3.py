...
#Incluindo os pesos para cada classe         
samples_per_class = {
            110:1000, # Wheat
            120:2000, #Maize
            140:1000, #Sorghum
...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        if acc > best_acc:
            best_acc = acc
            # Salvando o modelo
            torch.save(model.state_dict(), "model_2019_cat_cen3.pt")
            # Exportando o scaler
            joblib.dump(scaler, 'scaler_2019_cat_cen3.pkl')
            # Salvar encoder
            joblib.dump(linear_encoder, 'linear_encoder_2019_cat_cen3.pkl')
    ...
    np.save('ytrue_2019_cat_cen3.npy', np.array(all_true_total))
    np.save('ypred_2019_cat_cen3.npy', np.array(all_preds_total))

    # Criar .txt com resultados
    with open("resultado_2019_cat_cen3.txt", "w") as f:
