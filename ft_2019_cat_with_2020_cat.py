#Fine tuning do cat_2019 com cat_2020
...
def main():
    ...
    linear_encoder = joblib.load('linear_encoder_2019_cat.pkl')
    ...
    scaler = joblib.load('scaler_2019_cat.pkl')
    ...
    with torch.no_grad():
        ...
        if f1_macro > best_f1:
            best_f1 = f1_macro
            #Salvando o modelo
            torch.save(model.state_dict(), 'model_ft_2019_cat_with_cat_2020.pt')

    #Salvar resultados para gráficos e tabelas
    np.save('ytrue_2019_cat_with_2020_cat.npy', np.array(all_true_total))
    np.save('ypred_2019_cat_with_2020_cat.npy', np.array(all_preds_total))
    np.save('epoch_loss_2019_cat_with_2020_cat.npy',np.array(epoch_loss))
    #Gerar .txt com resultados
    with open('resultado_2019_cat_with_2020_cat.txt', 'w') as f:
