...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        if acc > best_acc:
            best_acc = acc
            # Exportar modelo
            torch.save(model.state_dict(), "lstm_kfold_best_2020_cat.pt")
            # Exportar scaler
            joblib.dump(scaler, 'scaler_lstm_kfold_2020_cat.pkl')
            # Salvar encoder
            joblib.dump(linear_encoder, 'linear_encoder_2020_cat.pkl')
    ...
    #Salvar trues e preds para os gráficos
    np.save('ytrue_2020_cat.npy', np.array(all_true_total))
    np.save('ypred_2020_cat.npy', np.array(all_preds_total))

    # Criar .txt com resultados
    with open("resultado_2020_cat_kfold.txt", "w") as f:
